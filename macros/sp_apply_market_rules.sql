...
-- prepare buckets to collect SQL for ordered execution (store pairs: log + query)
{% set global_inserts = [] %}
{% set global_deletes = [] %}
{% set local_inserts = [] %}
{% set local_deletes = [] %}
...
{% for grp_key, rule_data in grouped.items() %}
    ...
    {# --- DP + O joins --- #}
    {% if rule_data.HAS_MANUFACTURER == 1 or rule_data.HAS_CORPORATION == 1 or 'DP' in needed_aliases%}
